...
def calculate(particles, n, file_name)
  ...
  @walls = (0..LennardJonesParticle::WIDTH).step(1).map { |i| LennardJonesParticle.new(i, 0, 0, 0, true) } +
    (0..LennardJonesParticle::WIDTH).step(1).map { |i| LennardJonesParticle.new(i, LennardJonesParticle::HEIGHT, 0, 0, true) } +
    (1..LennardJonesParticle::HEIGHT - 1).step(1).map { |i| LennardJonesParticle.new(LennardJonesParticle::WIDTH, i, 0, 0, true) } +
    (1..LennardJonesParticle::HEIGHT - 1).step(1).map { |i| LennardJonesParticle.new(0, i, 0, 0, true) } +
    (1..90).step(1).map { |i| LennardJonesParticle.new(LennardJonesParticle::WIDTH / 2, i, 0, 0, true) } +
    (110..199).step(1).map { |i| LennardJonesParticle.new(LennardJonesParticle::WIDTH / 2, i, 0, 0, true) }
  ...
  while t < n do
    ...
    particles_current.each_with_index do |p, i|
      ...
      # part = LennardJonesParticle.new(x, y, p.vx, p.vy)
      # part.force(particles_current + @walls)
...
def print(particles, file)
  ...
  if file 
    ...
    particles.each { |p| file.write("#{a+=1}\t#{p.x}\t#{p.y}\n") } 
    @walls.each { |p| file.write("0\t#{p.x}\t#{p.y}\n") } 
  else
